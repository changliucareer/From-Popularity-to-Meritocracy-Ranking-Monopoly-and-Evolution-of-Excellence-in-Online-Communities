...
def myAction (commName, commDir, year, rank2commentCount,logFileName):
    logtext = f'for year {year},\n'
    # fill up rank2commentCount with ranks without vote
    # sort
    rank2commentCount = dict(sorted(rank2commentCount.items()))
    Longest = max(list(rank2commentCount.keys()))

    ranks = []
    for r in range(1,Longest+1):
        if r in rank2commentCount.keys():
            ranks.append(rank2commentCount[r])
        else:
            ranks.append(0)
    ...
    axes1.set_ylabel('Comment Proportion',fontsize=Fontsize)
    ...
    axes1.plot(x, simplifiedpowerlawFuncForMLE(spl_estParams[0],spl_estParams[1]), 'o-',
        label='Simplified PowerLaw MLE:\nb=%5.3f,c=%5.3f\n avgRes=%5.5f' % (spl_estParams[0],spl_estParams[1],spl_avgRes) )
    ...
    axes1.legend(fontsize=30)
    plt.setp(axes1.get_xticklabels(),fontsize=20)
    plt.setp(axes1.get_yticklabels(),fontsize=20)
    
    # save plot
    if commName =='MEAN':
        savePlot(fig1, f'MEAN_commentProportionAtEachRank_SimplifiedPowerLaw_Till{year}.pdf')
    else:
        savePlot(fig1, f'commentProportionAtEachRank_SimplifiedPowerLaw_Till{year}.pdf')
...
def myFun(commName, commDir, return_trendiness_dict, root_dir):
    ...
    logFileName = 'descriptive13_commentTrendiness_log.txt'
    ...
    with open(intermediate_directory+'/'+'yearlycommentCountAtEachRank.dict', 'rb') as inputFile:
        year2commentCountAtEachRank_total = pickle.load( inputFile)
    ...
    with mp.Pool(processes=n_proc) as pool:
        args = []
        for year, rank2commentCount in year2commentCountAtEachRank_total.items():
            args.append((commName,commDir, year,rank2commentCount,logFileName))
    ...
    with open('descriptive13_commentTrendinessFittingResults_allComm.dict', 'wb') as outputFile:
...
def main():
    ...
    # Report progress.
    print('descriptive 13 trendiness fitting of comment count Done completely.    Elapsed: {:}.\n'.format(elapsed))
