...
def plotAverageVoteCountAtEachVoteDiffJumpInterval(commName,commDir,total_vdInterval2voteAccumulatedCount,logFileName, vd_jump_step_size, first_interval, last_invertal, startAtRank):
    ...
    fig.suptitle(f'vote difference interval size {vd_jump_step_size}, start at rank {startAtRank}')

    # plt.ylim(ymin=min(y),ymax=max(y)+0.02)
    # plt.xlim(-64,xmax=100)

    picFileName = f'average_voteCount_atEachVoteDiffJumpInterval_jumpSize{vd_jump_step_size}_startAtRank{startAtRank}.pdf'
...
def myFun(commName, commDir):
    ...
    vd_jump_step_size = 10 # the minimux jump size should be 3, because the first pos interval can only start from 1, if jump size=2, the first pos interval has to be (1,1) which may cause bug
    ...
    for i,tup in enumerate(answer2values.items()):
        ...
        for v in values:
            ...
            if (cur_vd in vd_intervals_dict.keys()) and (rank == starAtRank): # one of start vd
            ...
            elif (cur_vd in vd_intervals_dict.values()):
    ...
    with open(intermediate_directory+'/'+f'total_vdInterval2voteAccumulatedCount_jumpSize{vd_jump_step_size}_startAtRank{starAtRank}.dict', 'wb') as outputFile:
        pickle.dump(total_vdInterval2voteAccumulatedCount, outputFile) 
        logtext = f"saved total_vdInterval2voteAccumulatedCount_jumpSize{vd_jump_step_size}_startAtRank{starAtRank} of {commName}. length: {len(total_vdInterval2voteAccumulatedCount)}\n"
        writeIntoLog(logtext, commDir, logFileName)
        print(logtext)
    
    """
    # load 
    with open(intermediate_directory+'/'+f'total_vdInterval2voteAccumulatedCount_jumpSize{vd_jump_step_size}_startAtRank{starAtRank}.dict', 'rb') as inputFile:
        total_vdInterval2voteAccumulatedCount = pickle.load( inputFile)
    
    # plot for averageVoteCountAtEachVoteDiffJumpInterval
    plotAverageVoteCountAtEachVoteDiffJumpInterval(commName,commDir,total_vdInterval2voteAccumulatedCount,logFileName, vd_jump_step_size, list(vd_intervals_dict_comm.items())[0], list(vd_intervals_dict_comm.items())[-1], starAtRank)
...
def main():
    ...
    # selected communities to run
    selected_comms = ['unix.meta.stackexchange','mathoverflow.net','politics.stackexchange']
    ...
    for tup in commDir_sizes_sortedlist:
        ...
        if commName not in selected_comms:
            print(f"{commName} isn't selected.")
            continue
        try:
