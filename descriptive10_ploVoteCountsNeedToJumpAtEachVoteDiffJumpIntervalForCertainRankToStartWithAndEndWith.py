...
def plotAverageVoteCountAtEachVoteDiffJumpInterval(commName,commDir,total_vdInterval2voteAccumulatedCount,logFileName, vd_jump_step_size, first_interval, last_invertal, startAndEndWithRank):
    ...
    fig.suptitle(f'vote difference interval size {vd_jump_step_size} start and end with rank {startAndEndWithRank}')

    # plt.ylim(ymin=min(y),ymax=max(y)+0.02)
    # plt.xlim(-64,xmax=100)

    picFileName = f'average_voteCount_atEachVoteDiffJumpInterval_jumpSize{vd_jump_step_size}_startAndEndWithRank{startAndEndWithRank}.pdf'
...
def myFun(commName, commDir):
    ...
    for i,tup in enumerate(answer2values.items()):
        ...
        for v in values:
            ...
            if (cur_vd in vd_intervals_dict.keys()) and (rank == startAndEndWithRank): # one of start vd
            ...
            elif (cur_vd in vd_intervals_dict.values()) and (rank == startAndEndWithRank): # one of end vd
    ...
    with open(intermediate_directory+'/'+f'total_vdInterval2voteAccumulatedCount_jumpSize{vd_jump_step_size}_startAndEndWithRank{startAndEndWithRank}.dict', 'wb') as outputFile:
        pickle.dump(total_vdInterval2voteAccumulatedCount, outputFile) 
        logtext = f"saved total_vdInterval2voteAccumulatedCount_jumpSize{vd_jump_step_size}_startAndEndWithRank{startAndEndWithRank} of {commName}. length: {len(total_vdInterval2voteAccumulatedCount)}\n"
        writeIntoLog(logtext, commDir, logFileName)
        print(logtext)
    
    
    # load 
    with open(intermediate_directory+'/'+f'total_vdInterval2voteAccumulatedCount_jumpSize{vd_jump_step_size}_startAndEndWithRank{startAndEndWithRank}.dict', 'rb') as inputFile:
        total_vdInterval2voteAccumulatedCount = pickle.load( inputFile)
    
    # plot for averageVoteCountAtEachVoteDiffJumpInterval
    plotAverageVoteCountAtEachVoteDiffJumpInterval(commName,commDir,total_vdInterval2voteAccumulatedCount,logFileName, vd_jump_step_size, list(vd_intervals_dict_comm.items())[0], list(vd_intervals_dict_comm.items())[-1], startAndEndWithRank)
